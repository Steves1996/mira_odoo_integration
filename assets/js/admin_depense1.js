...
function getTypeVehicule3_1(camion1,type1){
...
function getDistanceParCodeCamion1(camion1,destination1,destination2){
...
function getLitrageCamion1(destination1,type1,gazoil){
	
  
   $.ajax({
      type:"POST",
      url:base_url+"/admin_depense/getLitrageCamion1",
      data:{'type1':type1,'destination1':destination1},
      success: function(data){
...
function getLitrageCamion2(destination1,type1,gazoil1){
...
function getFraisRoute1(destination1,type1,route){
...
function getFraisRetour1(destination1,type1,retour){
...
if  (date_demande == ""){
		...
		$('.destination1'+i).css("border","red 2px solid");
			    toastr.error("Veuillez remplir tous les Champs");
				 return;
		...
		$('.destination1'+i).css("border","1px solid #ced4da");
		...
		destination1[i] = $('.destination1'+i).val();
...
// alert(status);
      ...
      data:{"status":status,"etat_demande":etat_demande,"etat_demande1":etat_demande1,"po":po,"date_demande":date_demande,"nbreLignes":nbreLigne,"rj":rj,"rj1":rj1,'id_demande':JSON.stringify(id_demande),'delegue':JSON.stringify(delegue),'bl':JSON.stringify(bl),'camion1':JSON.stringify(camion1),'immatriculation1':JSON.stringify(immatriculation1),'client':JSON.stringify(client),'destination1':JSON.stringify(destination1),'destination2':JSON.stringify(destination2),'route':JSON.stringify(route),'pont':JSON.stringify(pont),'retour':JSON.stringify(retour),'type1':JSON.stringify(type1),'gazoil':JSON.stringify(gazoil),'gazoil1':JSON.stringify(gazoil1),'marchandiseD':JSON.stringify(marchandiseD),'marchandiseR':JSON.stringify(marchandiseR),'kilometrage':JSON.stringify(kilometrage),'tour':JSON.stringify(tour),'tonnage':JSON.stringify(tonnage),'rjFR':JSON.stringify(rjFR),'rjFT':JSON.stringify(rjFT),'rjP':JSON.stringify(rjP)},
      success: function(data){
      	if ($.trim(data) == "Insertion parfaite de la demande") {
		   ...
		   //   title: 'Success',
		   //  subtitle: 'Alert',
		   //   body: 'Demande '+po+' en attente de validation '
		   //  }) 
			//	alert('Demande '+po+' à été modifié ');		  
                        }
...
//		$('.fournisseur').css("border","1px solid #ced4da");
	...
	if (delegue.length >nbreLigne && camion1.length>nbreLigne && destination1.length>nbreLigne && litrage.length>nbreLigne) {
$(".chargementPrime1").fadeIn();
// alert(status);
	   $.ajax({
      type:"POST",
      url:base_url+"/admin_depense/addDemandeG",
      data:{"status":status,"po":po,"date_demande":date_demande,"etat_demande":date_demande,"nbreLignes":nbreLigne,"rj":rj,'id_demande':JSON.stringify(id_demande),'delegue':JSON.stringify(delegue),'bl':JSON.stringify(bl),'camion1':JSON.stringify(camion1),'immatriculation1':JSON.stringify(immatriculation1),'complement':JSON.stringify(complement),'destination1':JSON.stringify(destination1),'fournisseur':JSON.stringify(fournisseur),'litrage':JSON.stringify(litrage),'detail':JSON.stringify(detail),'type1':JSON.stringify(type1),'rjG':JSON.stringify(rjG),'con':JSON.stringify(con)},
      success: function(data){
      	if ($.trim(data) == "Insertion parfaite de la demande") {
		      		$(".chargementPrime1").fadeOut();
		      	 $(document).Toasts('create', {
...
function detailDemandeG(po,delegue,date_com,etat_exp,etat_exp1,montant,montant1,montant2,signatureDAF,signatureDGT,compteur){
	$(".po2").empty();
	$(".po2").append(po+'<br/><br/>');
