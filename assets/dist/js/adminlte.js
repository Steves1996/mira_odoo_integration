...
function getIdNotification(){
   $.getScript(base_url+"admin/getNbreNewNotification"
  ,
  function(data){
    i=0;
    char = data.split(',');
    ...
    do{
        ...
        delai = 8750;
             $.ajax({
      type:"POST",
      url:base_url+"admin/getUniqueNotification",
      data:{"id":char[i]},
...
setTimeout(getNotificationParTemps,10500);
    $.getScript(base_url+"admin/getNbreNewNotificationParTemps"
  ,
  function(data){
    i=0;
    char = data.split(',');
...
if (char[i]!= " ") {
             ...
             $.ajax({
      type:"POST",
      url:base_url+"admin/getUniqueNotificationParTemps",
      data:{"id":char[i]},
      success: function(dat){
         // alert(char[i]);
         $(document).Toasts('create', {
...
function nePlusAfficher(ref_vue_notification){

  $.ajax({
      type:"POST",
      url:base_url+"admin/nePlusAfficher",
      data:{"ref":ref_vue_notification},
...
if($.trim($(".compte").val())=='SUPERVISEUR' || $.trim($(".compte").val())=='nathan' )   {
				  
                        
//		getNotificationParTemps();
//getIdNotification(); 
