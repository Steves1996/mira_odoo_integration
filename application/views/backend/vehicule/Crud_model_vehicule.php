        ...
        foreach ($query1 as $row) {
          $getOperation = $this->db->query("SELECT * from operation where id_operation = ".$row['id_operation']." limit 1")->row();
            echo "<tr >
                    ...
                    <td>
                    <button type='button' onclick=\"infosLocationEngin('".$row['id_location']."','".$row['code']."','".$row['duree']."','".$row['montant']."','".$row['date_location']."','".$row['commentaire']."')\" class=' btn-primary btn-sm'><i class='nav-icon fas fa-edit'></i></button>
