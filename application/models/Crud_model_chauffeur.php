...
public function getIdentifiantUtilisateur($id_profil){
  ...
  $this->db->close();
            ...
            if (count($getCodeTracteur)>0) {
                ...
                <td style='color:red;'>Aucun véhicule</td>";
                  ...
                  echo"  <td>".$row['nom']."
                    ...
                    <td>".number_format($row['retenueSalarialeass'],0,',',' ')."</td>
                    <td>";
                if($this->session->userdata('vehicule_modification')=='true'){
                    echo"
                    <button type='button' onclick='infosChauffeur(\"".$row['id_chauffeur']."\",\"".$row['num_permis']."\",\"".$row['telephoneChauffeur']."\",\"".$row['telephoneAssistant']."\",\"".$row['nom']."\",\"".$row['nom_ass']."\",\"".$row['cni_chauff']."\",\"".$row['cni_ass']."\",\"".$row['date_exp_cni_ass']."\",\"".$row['date_exp_cni_ch']."\",\"".$row['date_exp_permis_ch']."\",\"".$row['photoChauffeur']."\",\"".$row['photoAssistant']."\",\"".number_format($row['salaire'],0,',',' ')."\",\"".number_format($row['salaire_ass'],0,',',' ')."\",\"".$row['date_init']."\",\"".$row['montant_init']."\",\"".number_format($row['retenueSalariale'],0,',',' ')."\",\"".number_format($row['retenueSalarialeass'],0,',',' ')."\");' class=' btn-primary btn-sm'><i class='nav-icon fas fa-edit'></i></button>";
                }
    ...
    $retenueSalarialeAss=$_POST['retenueSalarialeAss'];
    ...
    if ($status == "insert") {
        ...
        echo "Un autre assistant utilise déja ce numéro de cni Veuillez changer";
         ...
         $this->addImageChauffeur($nomChauffeur,$cniChauffeur,$telephoneChauffeur,$expirationCNIChauffeur,$expirationPermisChauffeur,$nomAssistant,$cniAssistant,$expirationCNIAssistant,$telephoneAssistant,$permisChauffeur,$salaire,$salaire_ass,$date_init,$montant_init,$retenueSalariale,$retenueSalarialeAss);
    }
    ...
    public function addImageChauffeur($nomChauffeur,$cniChauffeur,$telephoneChauffeur,$expirationCNIChauffeur,$expirationPermisChauffeur,$nomAssistant,$cniAssistant,$expirationCNIAssistant,$telephoneAssistant,$permisChauffeur,$salaire,$salaire_ass,$date_init,$montant_init,$retenueSalariale,$retenueSalarialeAss){
    ...
    $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à ajouté le chauffeur dénommé ".$nomChauffeur.", de CNI ".$cniChauffeur." et téléphone ".$telephone." le ".$date_notif." à ".$heure;

    $message2 = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à modifié le chauffeur dénommé ".$nomChauffeur.", de CNI ".$cniChauffeur." et téléphone ".$telephone." le ".$date_notif." à ".$heure;
    ...
    $retenueSalarialeAss = intval(preg_replace('/\s/','', $_POST["retenueSalarialeAss"]));
    $salaire = intval(preg_replace('/\s/','', $_POST["salaire"]));
    $salaire_ass = intval(preg_replace('/\s/','', $_POST["salaire_ass"]));
    ...
    $message2 = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à modifié le chauffeur dénommé ".$nomChauffeur.", de CNI ".$cniChauffeur." et téléphone ".$telephoneChauffeur." le ".$date_notif." à ".$heure;
            ...
            if (count($requete)>0) {
                ...
                if (count($requete)>0) {
                    ...
                    if($query1 == true){
                     ...
                     $query1 = $this->db->query("UPDATE chauffeur set nom='".$nomChauffeur."',cni_chauff='".$cniChauffeur."', date_exp_cni_ch=CAST('".$expirationCNIChauffeur."' AS DATE),date_exp_cni_ass=CAST('".$expirationCNIAssistant."' AS DATE), date_exp_permis_ch=CAST('".$expirationPermisChauffeur."' AS DATE), nom_ass='".$nomAssistant."', cni_ass='".$cniAssistant."', num_permis='".$permisChauffeur."', telephoneChauffeur=".$telephoneChauffeur.",telephoneAssistant=".$telephoneAssistant.",salaire=".$salaire.",salaire_ass=".$salaire_ass.",date_init=CAST('". $date_init."' AS DATE), montant_init = ".$montant_init.", retenueSalariale =".$retenueSalariale.", retenueSalarialeass =".$retenueSalarialeAss." where id_chauffeur=".$id_chauffeur."");
                    if($query1 == true){
                        echo "Modification parfaite du Chauffeur";
                        $this->notificationAjout($nom_table,addslashes($message2));
                ...
                if (count($requete)>0) {
                    ...
                    foreach ($requete as $row) {
                        ...
                        if ($row["id_chauffeur"] == $id_chauffeur) {
                            # code...
                            $query1 = $this->db->query("UPDATE chauffeur set nom='".$nomChauffeur."',cni_chauff='".$cniChauffeur."', date_exp_cni_ch=CAST('".$expirationCNIChauffeur."' AS DATE),date_exp_cni_ass=CAST('".$expirationCNIAssistant."' AS DATE), date_exp_permis_ch=CAST('".$expirationPermisChauffeur."' AS DATE), nom_ass='".$nomAssistant."', cni_ass='".$cniAssistant."', num_permis='".$permisChauffeur."', telephoneChauffeur=".$telephoneChauffeur.",telephoneAssistant=".$telephoneAssistant.",salaire=".$salaire.",salaire_ass=".$salaire_ass.",date_init=CAST('". $date_init."' AS DATE), montant_init = ".$montant_init.", retenueSalariale =".$retenueSalariale.",
                                retenueSalarialeass =".$retenueSalarialeAss." where id_chauffeur=".$id_chauffeur."");
                    if($query1 == true){
                        echo "Modification parfaite du Chauffeur";
...
public function leSelectChauffeur2(){
 ...
 public function leSelectChauffeurPourBalance(){
         $query1 = $this->db->query("SELECT * from chauffeur ")->result_array();
         if (count($query1)>0) {
             # code...leSelectChauffeurPourBalance
            foreach ($query1 as $row) {
 ...
 public function getNomAssistant($id_chauffeur){

        $getChauffeur = $this->db->query("SELECT * from chauffeur where id_chauffeur =".$id_chauffeur."")->row();

        if (count($getChauffeur)>0) {
            # code...
            return $getChauffeur->nom_ass;
        }else{
            return "Aucun";
        }
    $this->db->close();
    ...
    public function getCamionParChauffeur($id_chauffeur){
	   ...
	   public function getChauffeurParCamion($id_chauffeur){
        
        $getChaffeurTracteur = $this->db->query("SELECT * from tracteur where code = '".$id_chauffeur."'")->row();
        
        $getChauffeurCamionBenne = $this->db->query("SELECT * from camion_benne where code = '".$id_chauffeur."'")->row();

        $getChauffeurEngin = $this->db->query("SELECT * from engin where code = '".$id_chauffeur."'")->row();
         
         if (count($getChaffeurTracteur) >0) {
             # code...
           return $getChaffeurTracteur->id_chauffeur;
         }elseif (count($getChauffeurCamionBenne) > 0) {
             # code...
            return $getChauffeurCamionBenne->id_chauffeur;
         }elseif (count($getChauffeurEngin) >0) {
             # code...
            return $getChauffeurEngin->id_chauffeur;
    ...
    public function getSalaireChauffeur(){
       ...
       $nom_table = "imputation_salaire";
    $heure = date("H:i:s");
    $date_notif = date("d-m-Y");
    if ($cible == 'chauffeur') {
        # code...
        $corps = "le chauffeur dénommé ".$this->getNomChauffeur($id_chauffeur);
         }else{
        $corps = "l'assistant dénommé ".$this->getNomAssistant($id_chauffeur)." en service avec le chauffeur : ".$this->getNomChauffeur($id_chauffeur);
        }

    $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à ajouté une imputation d'un montant de : ".$montant." sur ".$corps." ".$date_notif." à ".$heure;

    $message2 = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à modifié une imputation d'un montant de : ".$montant." sur ".$corps." le ".$date_notif." à ".$heure;
    ...
    $nom_table = "reglement_imputation";
    $heure = date("H:i:s");
    $date_notif = date("d-m-Y");
    if ($cible == 'chauffeur') {
        # code...
    ...
    $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à règlé une imputation d'un montant de : ".$montant." sur ".$corps." ".$date_notif." à ".$heure;

    $message2 = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à modifié le règlement d'une imputation d'un montant de : ".$montant." sur ".$corps." le ".$date_notif." à ".$heure;
     ...
     public function getAllImputationSalaire(){
		 ...
		 if (isset($_POST["date_debut"]) && isset($_POST["date_fin"]) && isset($_POST["id_fournisseur2"])) {
            # code...
        $date_debut = $_POST['date_debut'];
        $date_fin = $_POST['date_fin'];
        $id_fournisseur2 = $_POST['id_fournisseur2'];
        


		if (!empty($date_fin) && !empty($date_debut) && isset($_POST["id_fournisseur2"])) {
     ...
     //       $query1 = $this->db->query("SELECT distinct po_com,id_fournisseur_article,date_com,date_crea,etat_exp,etat_recep,id_commande,id_article,code_camion,description from commande where id_fournisseur_article = ".$id_fournisseur1." and date_com between '".$date_debut."' and '".$date_fin."'  group by po_com ORDER BY date_com DESC ")->result_array();
		...
		if ($id_fournisseur2 == 'TOUT') {
            # code...
         $query1 = $this->db->query("SELECT * from  imputation_salaire where  date_imputation between '".$date_debut."' and '".$date_fin."'  order by date_imputation desc")->result_array();
        
		}else {
			
		  $query1 = $this->db->query("SELECT * from  imputation_salaire where id_chauffeur = ".$this->getChauffeurParCamion($id_fournisseur2)." and date_imputation between '".$date_debut."' and '".$date_fin."'  order by date_imputation desc ")->result_array();
			
		}	
		
		}else{
			
			  $query1 = $this->db->query('SELECT * from  imputation_salaire order by date_imputation desc limit 300')->result_array();
        ...
        foreach ($query1 as $row) {
            # code...
			 ...
			 $compteur1 = $compteur1 + $row['salaire'];
			 
			  $compteur2 = $compteur2 + $row['montant'];
			  
			   $compteur3 =   $compteur+1;
	...
	if (isset($_POST["date_debut"]) && isset($_POST["date_fin"]) && isset($_POST["id_fournisseur2"])) {
     ...
     //       $query1 = $this->db->query("SELECT distinct po_com,id_fournisseur_article,date_com,date_crea,etat_exp,etat_recep,id_commande,id_article,code_camion,description from commande where id_fournisseur_article = ".$id_fournisseur1." and date_com between '".$date_debut."' and '".$date_fin."'  group by po_com ORDER BY date_com DESC ")->result_array();
		...
		if ($id_fournisseur2 == 'TOUT') {
            # code...
         $query1 = $this->db->query("SELECT * from  reglement_imputation where  date_imputation between '".$date_debut."' and '".$date_fin."'  order by date_imputation desc")->result_array();
        
		}else {
			
		  $query1 = $this->db->query("SELECT * from  reglement_imputation where id_chauffeur = ".$this->getChauffeurParCamion($id_fournisseur2)." and date_imputation between '".$date_debut."' and '".$date_fin."'  order by date_imputation desc ")->result_array();
			
		}	
		
		}else{
			
			  $query1 = $this->db->query('SELECT * from  reglement_imputation order by date_imputation desc limit 300')->result_array();
 ...
 $compteur1 =  $compteur1  + $row['salaire'];
 $compteur2 =  $compteur2  + $row['montant'];
 $compteur3 =  $compteur3  + $row['regulation'];
...
public function getDateInitialParChauffeur(){
   ...
   public function getImmatriculationVehiculeChauffeur($id_chauffeur){
    ...
    public function getTypeVehicule2($id_chauffeur){
...
$montant = 0;
    ...
    public function getAllRegulationImputationSalaireChauffeur($id_chauffeur,$date_debut,$date_fin){
        $query = $this->db->query('SELECT regulation,cible,montant from reglement_imputation WHERE  date_imputation between "'.$date_debut.'" and "'.$date_fin.'" and id_chauffeur='.$id_chauffeur.'')->result_array();
$montant = 0;
        if (count($query>0)) {
            # code...
            foreach ($query as $row) {
...
$montant = 0;
    ...
    public function getAllRetenueSalaireAssistant($id_chauffeur,$date_debut,$date_fin){
        $query = $this->db->query('SELECT retenueSalarialeass from chauffeur WHERE id_chauffeur='.$id_chauffeur.'')->result_array();
$montant = 0;
        if (count($query>0)) {
            # code...
            foreach ($query as $row) {
                # code...
                $montant = $montant + $row['retenueSalarialeass'];
            }
    ...
    $this->db->close();
     ...
     public function addEtatSalaireChauffeur(){
        ...
        if ($status == "insert") {
           ...
           if ($type == 'salaire') {
            # code...
            $insertImputation = $this->db->query("INSERT INTO paiechauffeur value('',".$id_chauffeur.",now(),".$valeur.",0,0,0)");
                if ($insertImputation == true) {
                    # code...
            ...
            # code...
            $insertImputation = $this->db->query("INSERT INTO paiechauffeur value('',".$id_chauffeur.",now(),0,0,".$valeur.",0)");
                if ($insertImputation == true) {
                    # code...
                    echo "Opération réussie";
                }else{
            ...
            $insertImputation = $this->db->query("INSERT INTO paiechauffeur value('',".$id_chauffeur.",now(),0,".$valeur.",0,0)");
                if ($insertImputation == true) {
                    # code...
                    echo "Opération réussie";
                }else{
        ...
        }elseif ($type == 'salaire_ass') {
            # code...
            $update = $this->db->query("UPDATE paiechauffeur set  id_chauffeur =".$id_chauffeur.",salaire_ass = ".$valeur."   where ref=".$ref."");
                if ($update == true ) {
                    # code...
                    echo "modification réussie";
                }else{
        ...
        if (count($query1)>0) {
         ...
         if (count($query2) >0) {
            ...
            // if ($date == date("Y-m",strtotime($query2->datePaie))) {
                 ...
                 echo "<td>".$this->getNomChauffeur($row['id_chauffeur'])."</td>
				   ...
				   <td ><input type=\"checkbox\" name=\"\" class=\"form-control gps".$compteur."\" id=".$query2->ref." onclick='addEtatSalaireChauffeur(\"".$query2->ref."\",\"".$row['id_chauffeur']."\",\"salaire_gps\",\"gps".$compteur."\",\"update\")'";
                    if ($query2->salaire_gps == 1 && $query2->salaire == 0 && $query2->salaire_ass == 0 && $query2->gps == 0) {
                   # code...
                echo"checked=\"true\" ";
               }elseif ($query2->salaire_gps == 0 && $query2->salaire == 0 && $query2->salaire_ass == 0 && $query2->gps == 0) {
                   # code...
               }else{
                echo "disabled = \"true\"";
               }
            ...
            if ($query2->salaire == 1 &&  $query2->salaire_gps == 0 && $query2->gps == 0) {
			 ...
			 <td ><input type=\"checkbox\" name=\"\" class=\"form-control sal_ass".$compteur."\" id=".$query2->ref." onclick='addEtatSalaireChauffeur(\"".$query2->ref."\",\"".$row['id_chauffeur']."\",\"salaire_ass\",\"sal_ass".$compteur."\",\"update\")'"; 


            if ($query2->salaire_ass == 1 &&  $query2->salaire_gps == 0 && $query2->gps == 0) {
                   # code...
                echo"checked=\"true\" ";
               }elseif ($query2->salaire_gps == 0 && $query2->salaire_ass == 0 && $query2->gps == 0) {
                   # code...
               }else{
                echo "disabled = \"true\"";
               }
               ...
               if ($query2->salaire == 1 || $query2->salaire_gps == 1 || $query2->gps == 1) {
                ...
                echo"
                 ...
                 echo "<td>".$this->getNomChauffeur($row['id_chauffeur'])."</td>
                   ...
                   <td ><input type=\"checkbox\" name=\"\" class=\"form-control sal".$compteur."\" onclick='addEtatSalaireChauffeur(\"0\",\"".$row['id_chauffeur']."\",\"salaire_ass\",\"sal_ass".$compteur."\",\"insert\")'></td>
               ...
               if ($query2->salaire == 1 || $query2->salaire_gps == 1 || $query2->gps == 1) {
                ...
                echo"
                 ...
                 echo "<td>".$this->getNomChauffeur($row['id_chauffeur'])."</td>
                    <td ><input type=\"checkbox\" name=\"\" class=\"form-control gps".$compteur."\" onclick='addEtatSalaireChauffeur(\"0\",\"".$row['id_chauffeur']."\",\"salaire_gps\",\"gps".$compteur."\",\"insert\")'></td>
                    <td ><input type=\"checkbox\" name=\"\" class=\"form-control sal".$compteur."\" onclick='addEtatSalaireChauffeur(\"0\",\"".$row['id_chauffeur']."\",\"salaire\",\"sal".$compteur."\",\"insert\")'></td>
                    <td ><input type=\"checkbox\" name=\"\" class=\"form-control sal_ass".$compteur."\" onclick='addEtatSalaireChauffeur(\"0\",\"".$row['id_chauffeur']."\",\"salaire_ass\",\"sal_ass".$compteur."\",\"insert\")'></td>
...
public function getValidePaieAssistant($id_chauffeur,$date_debut,$date_fin){
    $query2 = $this->db->query('SELECT * from paiechauffeur where id_chauffeur = '.$id_chauffeur.' order by datePaie desc limit 1')->row();
    if (count($query2)>0) {
        # code...
        if ($query2->salaire_ass==1) {
            # code...
            return false;
        }else{
            return true;
    ...
    public function getPaiePersonnel(){
        ...
        $date_fin = $_POST["date_fin"];
         ...
         foreach ($getChauffeur as $row) {
            ...
            $npAssistant = $row['salaire_ass']-$this->getAllReglementImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin)-$this->getAllRetenueSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllRegulationImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
            ...
            if ($date_debut >= '2023-05-01') {
                // code...
                $npAssistant = $npAssistant +10000;

                $salaire_ass = $row['salaire_ass'] + 10000;

                // $npAssistant= $npAssistant + 10000;
            }else{
                $salaire_ass = $row['salaire_ass'];
            }
   ...
   if ($this->getValidePaieAssistant($row['id_chauffeur'],$date_debut,$date_fin) == false) {
        # code...
    }else{
        ...
        echo "<tr style='text-align:center; font-size: 12px;'>
            ...
            <td >".$this->getCodeVehiculeChauffeur($row['id_chauffeur'])."</td>
            <td >".$this->getTypeVehicule1($row['id_chauffeur'])."</td>
            <td>".number_format($salaire_ass,0,',',' ')."</td>
            <td>".$this->getAllRetenueSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin)."</td>
         ...
         $totalNp = $totalNp + $np+ $npAssistant;
...
public function getPaiePersonnelType(){
        ...
        $getType = $this->db->query('SELECT * from type_vehicule where id_type = '.$code_camion.' limit 1')->row();
          
          if (count($getType)>0) {
            
            $type = $getType->nom_type;
         ...
         $type1 = $this->getTypeVehicule2($row['id_chauffeur']);
    ...
    if ($this->getImmatriculationVehiculeChauffeur($row['id_chauffeur']) == false) {
        ...
        echo "<tr style='text-align:center; font-size: 12px;'>
            ...
            <td >".$this->getImmatriculationVehiculeChauffeur($row['id_chauffeur'])."</td>
            <td >".$this->getCodeVehiculeChauffeur($row['id_chauffeur'])."</td>
            <td >".$this->getTypeVehicule2($row['id_chauffeur'])."</td>
            <td>".number_format($row['salaire'],0,',',' ')."</td>
        ...
        echo "<tr style='text-align:center; font-size: 12px;'>
         ...
         $totalRegulation = $totalRegulation +$this->getAllRegulationImputationSalaireChauffeur($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllRegulationImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
         $totalNp = $totalNp + $np+ $npAssistant;
         $totalSalaire = $totalSalaire + $salaire_ass +$row['salaire'];
         $totalCredit = $totalCredit + $credit;
         $totalCreditAss = $totalCreditAss + $creditAss;
        $totalCredit + $totalCredit+$totalCreditAss;
		}
...
public function getSommeIputation(){
    ...
    echo "<tr style='text-align:center; font-size: 12px; color:red;'>
           ...
           <td></td>
            ...
            <td ></td>
            <td ></td>
           <td style='color:red;font-size: 20px; border: 2px solid black; font-weight: bold;'>".$totalSalaire."</td>
            <td style='color:red;font-size: 20px; border: 2px solid black; font-weight: bold;'>".$totalRetenue."</td>
           <td></td>
...
// total de la paie
    ...
    public function getTotalPaiePersonnel(){
 ...
 if ($this->getValidePaieAssistant($row['id_chauffeur'],$date_debut,$date_fin) == false) {
     $npAssistant = 0;
            ...
            $npAssistant = $row['salaire_ass']-$this->getAllRetenueSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin)-$this->getAllReglementImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllRegulationImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
    ...
    if ($this->getImmatriculationVehiculeChauffeur($row['id_chauffeur']) == false) {
		...
		if ($this->getValidePaieAssistant($row['id_chauffeur'],$date_debut,$date_fin) == false) {
         ...
         $totalNp = $totalNp + $np;
         $totalSalaire = $totalSalaire +$row['salaire'];
         ...
         $totalRetenue = $totalRetenue + $this->getAllRetenueSalaireChauffeur($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllRetenueSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
         $totalReglement = $totalReglement + $this->getAllReglementImputationSalaireChauffeur($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllReglementImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
         $totalRegulation = $totalRegulation +$this->getAllRegulationImputationSalaireChauffeur($row['id_chauffeur'],$date_debut,$date_fin)+$this->getAllRegulationImputationSalaireAssistant($row['id_chauffeur'],$date_debut,$date_fin);
         ...
         $totalCredit = $totalCredit + $credit+$creditAss;
         $totalCreditAss = $totalCreditAss + $creditAss;
        // $totalCredit + $totalCredit+$totalCreditAss;
		}
         }
 ...
 public function getTotalPaiePersonnelType(){
...
public function deleteChauffeur($table, $identifiant, $nom_id){
        ...
        if (count($getCamion)>0) {
          ...
          $heure = date("H:i:s");
          $date_notif = date("d-m-Y");
          $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à supprimé le chauffeur  appelé ".$getCamion->nom." de téléphone ".$getCamion->telephoneChauffeur." le ".$date_notif." à ".$heure;
...
public function deleteImputation($table, $identifiant, $nom_id){
        ...
        if (count($getCamion)>0) {
          ...
          $heure = date("H:i:s");
          $date_notif = date("d-m-Y");
          if ($getCamion->cible == 'chauffeur') {
              # code...
            $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à supprimé une imputation du ".$getCamion->date_imputation." sur le chauffeur ".$this->getNomChauffeur($getCamion->id_chauffeur)." d'un montant de ".$getCamion->montant." le ".$date_notif." à ".$heure;
          }else{
             $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à supprimé une imputation du ".$getCamion->date_imputation." sur l'assistant' ".$this->getNomAssistant($getCamion->id_chauffeur)." d'un montant de ".$getCamion->montant." le ".$date_notif." à ".$heure;
          }
...
public function deleteReglementImputation($table, $identifiant, $nom_id){
        ...
        if (count($getCamion)>0) {
          ...
          if ($getCamion->cible == 'chauffeur') {
              # code...
            $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à supprimé le règlement d'une imputation du ".$getCamion->date_imputation." sur le chauffeur ".$this->getNomChauffeur($getCamion->id_chauffeur)." d'un montant de ".$getCamion->montant." le ".$date_notif." à ".$heure;
          }else{
             $message = $this->getIdentifiantUtilisateur($this->session->userdata('id_profil'))." à supprimé le règlement d'une imputation du ".$getCamion->date_imputation." sur l'assistant' ".$this->getNomAssistant($getCamion->id_chauffeur)." d'un montant de ".$getCamion->montant." le ".$date_notif." à ".$heure;
          }
